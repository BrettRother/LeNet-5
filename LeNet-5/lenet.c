...
__global__ void ConvoluteValid(double *d_in, double *d_weight, double *d_out, size_t in_height, size_t in_width) {
	...
	//__shared__ double shm[LENGTH_FEATURE0][LENGTH_FEATURE0]; // input will be this size or smaller (32 x 32)
	...
	if (row < out_height && col < out_width) {
		...
		for (int i = 0; i < LENGTH_KERNEL; ++i) {
			for (int j = 0; j < LENGTH_KERNEL; ++j) {
				...
				//if (i_row > -1 && i_row < in_height && i_col >= 0 && i_col < in_width) {
					result += d_in[in_row * in_width + in_col] * d_weight[i * LENGTH_KERNEL + j];
				//}
			}
		}
...
__global__ void ActionAndBias(double *d_feature, size_t f_height, size_t f_width, double bias) {
	size_t col = blockDim.x * blockIdx.x + threadIdx.x;
	size_t row = blockDim.y * blockIdx.y + threadIdx.y;
...
void ConvoluteForward(double *d_in, double *d_out, double *d_weight, double *d_bias, int in_channels, int out_channels, int in_height, int in_width) {
	...
	cudaMemset(d_output, 0, out_channels * h_out * w_out * sizeof(double));

	int k_sq = LENGTH_KERNEL * LENGTH_KERNEL; // we'll need this for accessing memory
